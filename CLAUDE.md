...
- **交界需求（当前契约）**：Aevum B2 已登记七类 Ultreia Report；Ultreia dispatch v8 与 producer B2 outbox constraints 已部署并通过只读 acceptance，正常 Cron 已看到七个独立槽位与 HTTP 200。Ultreia 可自主发现并独立投递计划偏移、训练负荷、恢复风险、目标赛、训练偏好失效、训练进步和重复健康风险的隐私裁剪聚合。Report 不能写 Aevum memory、执行 Action、扩大 scope 或延长 retention；敏感类型由 Aevum 记录后进入 `needs_user`。Aevum B3 窄范围只读 Query responder v3 已部署为 ACTIVE；Aevum 只能通过独立 HMAC 通道请求目标赛、已审核训练偏好和 28 天训练状态的去隐私当前快照，不能直读 Ultreia 表、获取原始训练 / GPS / 笔记 / 健康数据或写训练数据。改变训练数据的 Action 必须回到 Ultreia 做最新状态、权限、风险和冲突检查。
