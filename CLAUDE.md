...
- **交界需求（当前契约）**：Ultreia 已通过 B1 Reporter 自动上报隐私裁剪后的 `training_state_change / repeated_plan_deviation`；其他 Report 类别仍只在 Ultreia 本地 shadow，必须先完成 Aevum B2 contract / policy 登记才能外发。Report 不能写 Aevum memory、执行 Action、扩大 scope 或延长 retention。窄范围只读 Query 尚未接通。改变训练数据的 Action 必须回到 Ultreia 做最新状态、权限、风险和冲突检查。
