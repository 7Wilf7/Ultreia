...
- **GitHub repo**：`https://github.com/7Wilf7/training-studio`（main 分支推送后 Vercel 自动构建）
- **PWA**：已通过 vite-plugin-pwa 配置（`autoUpdate` 模式，前端发版后用户重开即拿新版）
- **Android APK**：用 Capacitor 打包，分发走 GitHub Releases；APP 内有"检查更新"按钮，调 GitHub Releases API 比对版本
