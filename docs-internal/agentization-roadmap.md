...
2. 在 Aevum 先定义 standing policy、`agent_report.v1`、Memory TTL、审计/
   恢复和 Cron/Edge Function 运行形态。
3. 保留窄范围 Ultreia Query，同时增加独立夜间 Report 通道；先以 shadow
   mode 跑完整的 Ultreia → Aevum 自动判断链路，不执行产品数据写入。
4. 先开放 Aevum 自动记忆维护，再逐项开放有明确上限、可逆的 `guarded`
   Ultreia Action。
