...
- 自动更新会基于当前 active facts + 当前目标赛事 + 最近对话生成英文 + 中文事实建议，并以 `memory_update` Action Card 审核后启用；接受时把 AI 结果视为完整 active 快照，相近事实复用旧行，快照中消失的旧 active facts 自动归档，真正新事实才新增。
...
- 保存的 active fact 会作为 `[Memory Facts]` 进入 Coach / 周报上下文；旧 `[Long-term Memory]` 文本块不再发送。分类边界收紧为：伤病 / 健康、赛事页面没有表达的长期策略边界、训练偏好、教练风格、长期模式五类。目标赛事日期、A / B / C 优先级、距离、爬升和目标状态以 `races` 表为 source of truth，不重复写成 Memory。
...
目标任务对象入口已按轻量方案落到 AI Coach 设置页，用户界面显示为“当前焦点”。第一版不新增顶栏图标、不新建 schema，只读取现有目标赛、未来计划、计划偏差、恢复守门和赛前简报状态；界面把长期焦点（通常是下一场目标赛）和短期焦点（近期计划、赛前窗口或训练记录重点）分开展示，并列出系统正在观察的风险和下一步可确认动作。修改焦点通过源数据完成：长期焦点改 Races 里的目标赛，短期焦点改 Calendar 的未来计划 / 完成状态。从焦点打开赛前简报或一键综合调整时，新详情叠在当前焦点上方，关闭详情后仍回到焦点，避免返回链路跳回 AI Coach 主界面；二级详情卡必须使用不透明实底，不能透出下层焦点内容。收件箱的其他 tab 会合并 Memory 更新和赛前简报的 `agent_actions` 记录，作为轻量回看入口。后续如果 Settings tab 重新分子 tab，再考虑把 AI Coach 相关设置整体挪过去。
...
- 2026-06-30：Memory 更新审核从“重读完整快照”改为“只审核变化”。后台仍要求模型返回完整 active snapshot，以便清理过期事实；前端先把候选事实与现有 active facts 做相似度匹配，区分新增、更新、建议归档和已存在。保存时未变化事实自动保留，用户只需要确认变化项。
- 2026-06-30：Memory 更新加入当前目标赛事 source-of-truth 校验。手动更新和夜间审核都会把 Races 里的目标赛事传给模型；如果旧聊天与当前赛事优先级冲突，模型必须相信当前 Races。Memory 只保留赛事策略边界，例如 HYROX 不应取代越野主线，不保存 A / B / C 优先级、日期、距离和爬升这类结构化字段。
