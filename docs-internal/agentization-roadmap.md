...
目标任务对象入口已按轻量方案落到 AI Coach 设置页，用户界面显示为“当前焦点”。第一版不新增顶栏图标、不新建 schema，只读取现有目标赛、未来计划、计划偏差、恢复守门和赛前简报状态；界面把长期焦点（通常是下一场目标赛）和短期焦点（近期计划、赛前窗口或训练记录重点）分开展示，并列出系统正在观察的风险和下一步可确认动作。修改焦点通过源数据完成：长期焦点改 Races 里的目标赛，短期焦点改 Calendar 的未来计划 / 完成状态。从焦点打开赛前简报或一键综合调整时，新详情叠在当前焦点上方，关闭详情后仍回到焦点，避免返回链路跳回 AI Coach 主界面。收件箱的其他 tab 会合并 Memory 更新和赛前简报的 `agent_actions` 记录，作为轻量回看入口。后续如果 Settings tab 重新分子 tab，再考虑把 AI Coach 相关设置整体挪过去。
