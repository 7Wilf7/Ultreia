...
| Aevum Phase A 自治底座 | 已完成 | `agent_report.v1`、策略、派生记忆、Action 安全逻辑和客户端锁定的数据库基础已建立；尚无 ingress、adapter、scheduler 或 Action executor |
...
1. Calendar 客户端预检与后续多次写入之间仍有极窄 TOCTOU 窗口。彻底关闭
   需要数据库事务型 RPC 或等价的乐观并发约束；在另行评审 schema、完整
   SQL、调用与回滚方案之前，不假设该能力已经存在。
2. 先完成 Aevum Phase A 数据库的只读验收，核对对象、RLS、权限、函数、
   空表和现有 UI 无回归；验收期间不写入或放宽权限。
3. 实现独立的 `agent_report.v1` 夜间 Report adapter；明确 JS 契约与 SQL
   字段映射，不直接读取或复制训练私表。窄范围 Ultreia Query 可在同一阶段
   作为独立只读通道推进，但不阻塞主动 Report。
4. 先以 shadow mode 跑完整的 Ultreia → Aevum 校验、策略、去重、派生记忆
   分类和 Journal 链路，不执行产品数据写入。
5. shadow 数据稳定后先开放 Aevum 自动记忆维护，再逐项开放有明确上限、
   可逆的 `guarded` Ultreia Action。
