...
| Aevum B2 Report catalog | 已部署 | Aevum ingress v8 已登记七类；Ultreia 通过 detector registry 自主发现、privacy/schema gate、每类独立 outbox/lease/cadence 后投递。计划依从判断已合并进 `repeated_plan_deviation`，不会重复上报；恢复与健康风险 receipt 的 `needs_user` 视为成功投递 |
| Ultreia autonomous candidate discovery | 已上线 | domain snapshot → deterministic features → detector registry → 意义 / 新颖性 / 复发判断 → registered type → privacy / schema gate → independent outbox；当前完全使用确定性代码，`allowed_minimized` 仅保留未启用扩展点，不调用付费模型 |
...
1. 观察七类 live Report 的命中率、频率跳过、重复率、敏感隔离和隐私边界；
   确定性 4xx 必须 blocked，不能用新候选覆盖失败 envelope。
