...
- 2026-06-25：计划偏差补救和恢复负荷守门从“用户点击后生成”改为“命中后自动生成建议草案”；两类信号同时存在时合并成 `combined_training_adjustment`，恢复 / 负荷保护优先。`暂不处理` 按当前信号指纹抑制，只有训练、计划或恢复信号变化后才会再次出现；写入 Calendar 仍需用户确认。
