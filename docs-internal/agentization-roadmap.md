...
| Aevum B1 Report | 已部署并 recorded | 仅自动上报已登记的 `training_state_change / repeated_plan_deviation` 14 日聚合；2026-07-13 真实 canary 为 15 planned / 8 done / 7 missed，receipt=`recorded`；2026-07-15 修复本地窗口转 UTC、pending 不覆盖、paused / blocked / retry 和并发租约 |
| Ultreia autonomous candidate discovery | 本地 shadow | domain snapshot → deterministic features → detector registry → 意义 / 新颖性 / 复发判断 → catalog → privacy / schema gate；负荷、恢复、目标、偏好、里程碑和健康风险候选只写最小化 structured log，不投递 Aevum |
| Aevum B2 catalog / policy | 仅提案 | 精确类型、schema、阈值、敏感度、频率、保留和 needs_user 边界见 `agent-report-candidate-catalog-b2-handoff.md`；Aevum 登记前不得外发 |
...
1. 观察本地 shadow candidate 的命中率、重复率和隐私边界；B1 只允许现有登记
   类型进入 outbox，确定性 4xx 必须 blocked，不能用新候选覆盖失败 envelope。
2. Aevum 完成 B2 contract / policy 后，再逐类把 shadow / needs_user 提案改为 live；
   登记前不得扩大 type、字段、scope、sensitivity 或 retention。
3. shadow 数据稳定后再实体化 working / short / long 派生记忆并启用生命周期；
