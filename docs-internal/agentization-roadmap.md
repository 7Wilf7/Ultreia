...
目标任务对象入口已按轻量方案落到 AI Coach 设置页，用户界面显示为“当前焦点”。第一版不新增顶栏图标、不新建 schema，只读取现有目标赛、未来计划、计划偏差、恢复守门和赛前简报状态；界面把长期焦点（通常是下一场目标赛）和短期焦点（近期计划、赛前窗口或训练记录重点）分开展示，并列出系统正在观察的风险和下一步可确认动作。后续如果 Settings tab 重新分子 tab，再考虑把 AI Coach 相关设置整体挪过去。
