...
| Aevum B3 Query responder | 已部署 | v3 ACTIVE、`verify_jwt=false`；已分离 Supabase 运行路径与固定 HMAC 规范路径，仍使用独立 HMAC、精确契约和 300 秒时效门禁；只返回目标赛、已审核训练偏好与 28 天聚合训练状态，不外发原始训练、GPS、笔记、健康信息或内部 ID |
...
3. 观察 B3 Query 的契约失败率、响应大小和来源读取失败；Aevum 是否把 Query
   结果派生为 memory 仍完全由 Aevum standing policy 决定，Query 不授予 Action
   或 Ultreia 写权限。
