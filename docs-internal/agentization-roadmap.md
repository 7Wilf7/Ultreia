...
| Aevum shadow Report | 已部署并 recorded | 独立 Reporter 只上报 `training_state_change / repeated_plan_deviation` 的 14 日聚合；2026-07-13 真实 canary 为 15 planned / 8 done / 7 missed，Aevum receipt=`recorded`；不带训练明细、不调用 LLM、不写派生记忆或 Action |
...
1. 连续观察 source/content 水位、recorded/replayed/duplicate receipt、三段退避和
   24 小时暂停；shadow 期只允许写 Ultreia outbox 与 Aevum Report/Journal。
2. shadow 数据稳定后再实体化 working / short / long 派生记忆并启用生命周期；
   窄范围 Query 可独立推进，但不阻塞主动 Report。
3. 自动 `guarded` Calendar Action 继续后置；启用前先用事务型 RPC 或等价的
   乐观并发约束关闭客户端预检后的 TOCTOU 窗口。
4. 不增加新 Agent 类型，不继续扩张设置页或解释型矩阵 UI。
