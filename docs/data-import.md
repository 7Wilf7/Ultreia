...
4. 接着是 **Review** 面板，列出每行的解析结果和勾选框：
   - 不想导入的行取消勾选。
   - 每行第一行是日期 / 类型 / （Road Run 的）配速子类下拉，第二行是具体数据，**末尾有个小 RPE 框**可以直接填这次的自觉用力（1–10），省得导入后再逐条补——RPE 填得越全，训练负荷和 AI 教练越准。
   - Road Run 行可以通过下拉框覆盖自动分配的配速子类。
   - 如果导入的活动里有**最近 24 小时内**的，或有超长训练与最近 24 小时重叠，面板里会多出一个**获取天气**开关（默认开），给这些活动抓天气，用的是 FIT 文件里自带的 GPS 位置。完全早于 24 小时窗口的活动抓不到（气象服务只保留过去 24 小时），所以批量导入历史记录时不会抓也不显示这个开关。
5. 点 **Import** 写入数据库。
