...
5. 点 **Import** 后，会出现 **完成导入**弹窗：
   ...
   - 可以选择是否让 AI Coach 点评，并在文本框里写这次的主观感受。开启点评时，应用会先保存活动；保存成功后才弹出发送给教练的确认框，确认前不会消耗 AI 对话额度。聊天里会显示你的主观感受，后台会同时带上新增活动、当前计划和负荷信息，让教练在点评训练的同时给下一次相关计划一个统一口径。教练点评完成后，应用会结合你的原始感受和教练点评，压缩成短「自评」备注写回对应训练；训练列表和日历详情里最多显示两行。
   - 如果当天是周日，上传保存成功后会额外询问是否现在生成 AI 周复盘；点稍后不会自动分析。
6. 点 **Save import** 写入数据库。保存过程中同一批活动会被暂时视为重复，避免网络慢或重传时写出两份相同记录。
