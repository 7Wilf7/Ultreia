...
5. 点 **Import** 后，会出现 **完成导入**弹窗：
   ...
   - 可以选择是否让 AI Coach 点评，并在文本框里写这次的主观感受。开启点评时，应用会先保存活动；保存成功后才弹出发送给教练的确认框，确认前不会消耗 AI 对话额度。聊天里会显示你的主观感受，后台会同时带上新增活动、当前计划和负荷信息，让教练在点评训练的同时给下一次相关计划一个统一口径。你的原始主观感受会由应用本地压缩成短「自评」备注写回对应训练，不再让教练代写这条备注；训练列表和日历详情里最多显示两行。
