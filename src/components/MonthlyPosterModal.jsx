...
// Single dark finish. The current logo becomes a full-canvas watermark so the
// topographic lines and mountain-route mark stay recognisable without looking
// like a pasted cutout.
const THEMES = {
  night: {
    bg: "#050705",
    ink: "#f3f0e2",
...
// ── The poster ──────────────────────────────────────────────────────────────
function PosterBackground({ W, H, pal, logoSrc }) {
  ...
  return (
    <g style={{ pointerEvents: "none" }}>
      <image href={logoSrc} x={x} y={y} width={size} height={size}
        opacity={pal.markOpacity} preserveAspectRatio="xMidYMid meet" />
      <rect width={W} height={H} fill={pal.bg} opacity={pal.logoDimOpacity} />
    </g>
  );
}
...
function Poster({ stats, theme, ratio, svgRef, logoSrc }) {
  ...
  return (
    <svg viewBox={`0 0 ${W} ${H}`} width={W} height={H} xmlns="http://www.w3.org/2000/svg" ref={svgRef}
      ...
      <PosterBackground W={W} H={H} pal={pal} logoSrc={logoSrc} />
      ...
      <text x={W / 2} y={yUrl} textAnchor="middle" fontFamily={FF} fontWeight="600" fontSize="24" letterSpacing="1.5" fill={pal.sub}>ultreia.run</text>
    </svg>
  );
}
...
export function MonthlyPosterModal({ logs, races = [], onClose }) {
  ...
  // URL, which the PNG export path requires (see fetchAsDataUrl). The same logo
  // is also used as the dimmed poster background.
  ...
  async function downloadPoster() {
    ...
    setMsg("");
    try {
      // Every <image> href must be a data URL before serialization (an
      // <img>-loaded SVG can't fetch external resources). Await the cached
      // conversion — instant when the mount effect already finished — and flush
      // it into the DOM so the serializer sees it.
