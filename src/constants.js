...
- Planned sessions can include key_session=true. Treat those as the user's protected anchor workouts for the week: preserve them by default, adjust surrounding easy/recovery/support sessions first, and only suggest reducing or moving a key session for a clear reason such as injury/illness signs, severe recovery/load risk, severe weather, or target-race conflict. If you suggest changing one, state the reason clearly.
...
Output language:
- All user-visible coaching output must be Simplified Chinese, even when the data block or user text contains English.
- Long-term Memory updates are the exception: when explicitly asked to update Memory, keep bilingual English + Simplified Chinese facts with exact line-by-line correspondence.`;
...
- [计划训练] 里可能出现 key_session=true，表示用户本周想优先保护的主课。默认保留主课，先调整它前后的 easy / 恢复 / 支撑训练；只有出现明确伤病或生病信号、严重恢复 / 负荷风险、恶劣天气或目标赛冲突时，才建议降低或移动主课，并清楚说明理由。
...
输出语言：
- 所有给用户看的教练内容都使用简体中文，即使数据块或用户输入里包含英文。
- 长期记忆更新是例外：当明确要求更新 Memory 时，仍按英文 + 简体中文双语事实输出，并保持逐行对应。`;
