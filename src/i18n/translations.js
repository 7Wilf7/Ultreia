...
const dict = {
  ...
  "weekly_settings.auto_generate_desc": { en: "Runs once per selected week while the app is active.", zh: "App 活跃时按选择的星期和时间每周生成一次。" },
  ...
  "weekly_settings.after_sunday_import": { en: "Ask after Sunday imports", zh: "周日导入后询问" },
  "weekly_settings.after_sunday_import_desc": { en: "After uploading activities on Sunday, ask whether to analyze the week now.", zh: "周日上传活动后，询问是否现在分析本周周报。" },
  "weekly_settings.behavior_note": { en: "This is an in-app schedule: it checks when Ultreia is open or returns to the foreground. It will not run secretly if the phone has killed the app.", zh: "这是 App 内定时：打开 Ultreia 或回到前台时检查是否到点。手机系统如果已经杀掉后台，它不会偷偷运行。" },
  ...
  "coach.memory_action_reason": { en: "This action will save the kept points as the coach's long-term Memory for future chats.", zh: "这个动作会把你保留的条目写入教练的长期记忆，供之后对话使用。" },
  "coach.memory_action_executed": { en: "Memory action executed. The accepted points are now saved.", zh: "记忆动作已执行。已接受的条目已经保存。" },
  "coach.memory_action_rejected": { en: "Memory action ignored. No Memory changes were saved.", zh: "记忆动作已忽略。长期记忆没有改动。" },
  "coach.memory_facts_title": { en: "Memory facts", zh: "记忆事实" },
  "coach.memory_facts_hint": { en: "Reviewed fact cards saved from accepted Memory updates. They are a sidecar layer for review and later agent use; the text Memory above remains the current prompt fallback.", zh: "这里是从已接受 Memory 更新中保存的事实卡片，先作为旁路审核层；上方整段 Memory 仍然是当前 Prompt 的兜底记忆。" },
  ...
  "coach.agent_action_memory_detail": { en: "{count} memory version(s)", zh: "{count} 个记忆版本" },
